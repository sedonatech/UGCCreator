...
const useAITools = (toolType = 'scripts') => {
    ...
    const handleSaveAndSubmit = async () => {
        try {
            ...
            const prompt =
                toolType === 'scripts' ? scriptPrompt : toolType === 'hooks' ? hooksPrompt : contentSuggestionsPrompt;
            ...
            const contentGenerationResultsHistoryParsed =
                (await JSON.parse(contentGenerationResultsFromLocalStorage)) || [];

            await AsyncStorage.setItem(
                'contentGenerationResults',
                JSON.stringify([
                    ...contentGenerationResultsHistoryParsed,
                    {
                        type: toolType,
