            ...
            try {
                ...
                if (allOfferings?.length) {
                    ...
                    if (featureCurrentOffering && !!currentOffering) {
                        const currentOfferingPackageProductIDs = currentOffering?.availablePackages?.map(
                    ...
                    const availableSortedPackages = sortOrderedPackages.map(pack => {
                        ...
                        console.log('🚀 ~ introPrice:', introPrice);
                        ...
                        const salePriceString =
                            !isDiscountSale && isIntroSale ? introPriceString : discount?.priceString;

                        const savingPercent = isDiscountSale
                            ? discountPercent
                            : isIntroSale
                            ? introSavingPercent
                            : null;
                        ...
                        const originalIdentifier = sortOrder?.find(
                            sortIdentifier => appPurchase?.metaData[sortIdentifier]?.discountKey === identifier,
                        );
                        ...
                        return {
                            ...
                            originalPrice: isDiscountSale
                                ? originalPriceString
                                : isIntroSale
                                ? originalIntroPriceString
                                : null,
                            title: product?.title,
                            ...
                            savingPercent: savingPercent
                                ? roundDownNumber(savingPercent, appPurchase?.roundSavingPercentage)
                                : 0,
                            discountIntroPriceString,
                            introPriceCycle,
                            periodNumberOfUnits,
