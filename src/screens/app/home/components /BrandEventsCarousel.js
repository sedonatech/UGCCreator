...
const BrandEventsCarousel = ({ style, brandId }) => {
    ...
    if (!eventsData?.length)
        return (
            <ProfileStatusCard
                title={t('brands.admin.carousels.events.noEvents.title')}
                description={t('brands.admin.carousels.events.noEvents.description')}
                showProgress={false}
                showIcon={false}
            ...
            <TemplateBox row alignItems="center" ph={WRAPPER_MARGIN}>
                ...
                <TemplateTouchable onPress={() => navigation.navigate(BRAND_EVENTS_SCREEN)}>
                    <TemplateText startCase size={14} underLine color={IOS_BLUE}>
                        {t('brands.admin.carousels.events.seeAll')}
                    </TemplateText>
                </TemplateTouchable>
            </TemplateBox>
