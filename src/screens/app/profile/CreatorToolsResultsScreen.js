...
import { BLACK_SECONDARY, GREEN, IOS_BLUE, LIGHT_GREEN, PAYWALL_PRIMARY_BACKGROUND } from '../../../theme/Colors';
...
const CreatorToolsResultsScreen = ({ navigation, route }) => {
    ...
    const { responseMessage } = useAITools();
    ...
    useEffect(() => {
        if (copied) {
            Alert.alert(t('common.alerts.copiedToClipboard.title'), t('common.alerts.copiedToClipboard.message'), [
                {
            ...
            <TemplateBox mt={HEADER_MARGIN} selfCenter mb={50}>
                ...
                {results && (
                    <TemplateBox
                        ...
                        borderRadius={16}
                    >
                        <TemplateText size={13}>{results}</TemplateText>
                    </TemplateBox>
                )}
