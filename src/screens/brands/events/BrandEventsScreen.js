...
import {BRAND_EVENT_DETAILS_SCREEN, BRAND_EVENTS_COMPLETED_SCREEN, EVENT_DETAILS_SCREEN, FAVORITE_EVENTS_SCREEN} from '../../../navigation/ScreenNames';
...
const BrandEventsScreen = ({navigation}) => {
  ...
  const filteredEvents = useMemo(() => {
    ...
    return search?.length ? searchResults?.map(event => ({
      id: event?.id,
      name: event?.title,
      description: event?.description,
      image: event?.image,
...
useLayoutEffect(() => {
  navigation.setOptions({
      headerRight: () => (
        <TemplateBox mr={wp(12)} ph={wp(12)} pv={hp(8)} justifyContent='center' alignItems='center' borderRadius={8} backgroundColor={BRAND_BLUE}  
        onPress={()=> navigation.navigate(BRAND_EVENTS_COMPLETED_SCREEN)}
        >
